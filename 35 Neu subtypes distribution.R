...
class.boundary <- dcast(spatil.layer.sub,subtype~Boundary_Class)
rownames(class.boundary) <- class.boundary$subtype
class.boundary <- class.boundary[,-1]
class.boundary <- class.boundary[,c('Edge','Transition','Core')]
rownames(class.boundary)

class.boundary <- class.boundary[c(46:52),] #Neu
a <- class.boundary
...
bk <- c(seq(0.5,1,by=0.01),seq(1.1,1.3,by=0.01))
library(pheatmap)
